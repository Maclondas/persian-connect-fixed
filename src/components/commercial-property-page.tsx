...
const subcategories = [
  { icon: Store, name: 'Shops for Rent / Sale', namePersian: 'مغازه برای اجاره / فروش', color: 'bg-blue-500' },
  { icon: Building2, name: 'Offices & Workspaces', namePersian: 'دفاتر و فضای کاری', color: 'bg-purple-500' },
  { icon: Warehouse, name: 'Warehouses & Storage Units', namePersian: 'انبار و واحدهای ذخیره‌سازی', color: 'bg-green-500' },
  { icon: Scissors, name: 'Salons & Barber Shops', namePersian: 'سالن‌ها و آرایشگاه‌ها', color: 'bg-pink-500' },
  { icon: DollarSign, name: 'Business for Sale', namePersian: 'کسب و کار برای فروش', color: 'bg-red-500' },
  { icon: TrendingUp, name: 'Franchise & Investment Opportunities', namePersian: 'فرنچایز و فرصت‌های سرمایه‌گذاری', color: 'bg-orange-500' }
];

export function CommercialPropertyPage({ onNavigate }: CommercialPropertyPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [commercialAds, setCommercialAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadCommercialAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('commercial-property');
        setCommercialAds(ads);
      } catch (error) {
        console.error('Failed to load commercial property ads:', error);
        toast.error('Failed to load commercial properties');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedProperties = () => {
    const properties = [...commercialAds];
    ...
    switch (selectedFilter) {
      ...
      case 'urgent':
        return properties.sort((a, b) => (b.isUrgent ? 1 : 0) - (a.isUrgent ? 1 : 0));
      case 'negotiable':
        return properties.sort((a, b) => (b.negotiable ? 1 : 0) - (a.negotiable ? 1 : 0));
      default:
        return properties;
    }
  };
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Commercial Property' : 'املاک تجاری'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search commercial properties...' : 'جستجوی املاک تجاری...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Categories' : 'مرور دسته‌بندی‌ها'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Commercial Properties' : 'املاک تجاری ویژه'}
          </h2>
        ...
        {/* Featured Commercial Properties Grid */}
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ...
                  : 'هیچ ملک تجاری یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a commercial property!' 
                  : 'اولین نفری باشید که ملک تجاری ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {getSortedProperties().map((ad) => (
                <div 
                  key={ad.id} 
                ...
                >
                  ...
                  <div className="p-3">
                    ...
                    <p className="text-xs text-gray-500 mb-1">{ad.subcategory || ad.category}</p>
                    {ad.views && (
                      <p className="text-xs text-gray-500 mb-2">{ad.views} views</p>
                    )}
