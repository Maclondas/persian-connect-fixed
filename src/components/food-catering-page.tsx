...
import { ArrowLeft, Search, ChefHat, Utensils, Cookie, ShoppingBasket, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner';
import { realDataService, Ad } from './services/real-data-service';

interface FoodCateringPageProps {
...
const subcategories = [
  { icon: ChefHat, name: 'Persian Restaurants & Takeaway', namePersian: 'رستوران و غذای آماده ایرانی', color: 'bg-red-500' },
  { icon: Utensils, name: 'Catering Services', namePersian: 'خدمات پذیرایی', color: 'bg-blue-500' },
  { icon: Cookie, name: 'Homemade Food & Sweets', namePersian: 'غذا و شیرینی خانگی', color: 'bg-pink-500' },
  { icon: ShoppingBasket, name: 'Grocery Stores & Specialty Shops', namePersian: 'فروشگاه مواد غذایی', color: 'bg-green-500' }
];

export function FoodCateringPage({ onNavigate }: FoodCateringPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [foodAds, setFoodAds] = useState<Ad[]>([]);
  ...
  useEffect(() => {
    const loadFoodAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('food-catering');
        setFoodAds(ads);
      } catch (error) {
        console.error('Failed to load food & catering ads:', error);
        toast.error('Failed to load food & catering ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedFoodServices = () => {
    return [...foodAds].sort((a, b) => {
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Food & Catering' : 'غذا و پذیرایی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search food & catering...' : 'جستجوی غذا و پذیرایی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Food Categories' : 'مرور دسته‌بندی غذا و پذیرایی'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Food Services' : 'خدمات غذا و پذیرایی ویژه'}
          </h2>
        ...
        {/* Featured Food Services Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : getSortedFoodServices().length === 0 ? (
            <div className="text-center py-12">
              <ChefHat className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No food & catering services found' 
                  : 'هیچ سرویس غذا و پذیرایی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a food & catering service!' 
                  : 'اولین نفری باشید که سرویس غذا و پذیرایی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedFoodServices().map((service) => (
                <div 
                  key={service.id} 
