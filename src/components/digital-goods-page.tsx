...
const subcategories = [
  { icon: Smartphone, name: 'Mobile Phones', namePersian: 'تلفن همراه', color: 'bg-blue-500' },
  { icon: Laptop, name: 'Laptops & Computers', namePersian: 'لپ‌تاپ و کامپیوتر', color: 'bg-green-500' },
  { icon: Tablet, name: 'Tablets', namePersian: 'تبلت', color: 'bg-orange-500' },
  { icon: Gamepad2, name: 'Gaming Consoles & Games', namePersian: 'کنسول و بازی', color: 'bg-purple-500' },
  { icon: Headphones, name: 'Audio & Headphones', namePersian: 'صوتی و هدفون', color: 'bg-red-500' },
  { icon: Camera, name: 'Cameras & Photography', namePersian: 'دوربین و عکاسی', color: 'bg-indigo-500' },
  { icon: Home, name: 'Smart Home Devices', namePersian: 'دستگاه‌های خانه هوشمند', color: 'bg-cyan-500' },
  { icon: Cable, name: 'Accessories & Cables', namePersian: 'لوازم جانبی و کابل', color: 'bg-gray-500' }
];
...
export function DigitalGoodsPage({ onNavigate }: DigitalGoodsPageProps) {
  ...
  const [digitalGoodsAds, setDigitalGoodsAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadDigitalGoodsAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('digital-goods');
        setDigitalGoodsAds(ads);
      } catch (error) {
        console.error('Failed to load digital goods ads:', error);
        toast.error('Failed to load digital goods ads');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Digital Goods (Electronics)' : 'کالاهای دیجیتال (الکترونیک)'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search electronics...' : 'جستجوی الکترونیک...'}
      ...
      <div className="bg-white">
        ...
        <div className="px-4 py-4">
          ...
          ) : digitalGoodsAds.length === 0 ? (
            <div className="text-center py-12">
              <Smartphone className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No digital goods found' 
                  : 'هیچ کالای دیجیتالی یافت نشد'
                }
              </p>
