...
import { ArrowLeft, Search, Dumbbell, Users, Monitor, Target, Zap, Activity, Heart, Utensils, Building, Baby, Shield, Trophy, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
...
const subcategories = [
  { icon: Dumbbell, name: 'Personal Training', namePersian: 'تمرینات شخصی', color: 'bg-blue-500' },
  { icon: Users, name: 'Group Fitness Training', namePersian: 'تمرینات گروهی', color: 'bg-green-500' },
  { icon: Monitor, name: 'Online / Virtual Coaching', namePersian: 'مربیگری آنلاین', color: 'bg-purple-500' },
  { icon: Target, name: 'Weight Loss & Fat Burning Programs', namePersian: 'برنامه کاهش وزن', color: 'bg-red-500' },
  { icon: Zap, name: 'Strength & Muscle Building', namePersian: 'قدرت و عضله‌سازی', color: 'bg-orange-500' },
  { icon: Activity, name: 'Functional Fitness & Mobility', namePersian: 'تناسب اندام عملکردی', color: 'bg-indigo-500' },
  { icon: Heart, name: 'Cardio & Endurance Training', namePersian: 'تمرینات قلبی و استقامت', color: 'bg-pink-500' },
  { icon: Activity, name: 'Yoga & Pilates Coaching', namePersian: 'یوگا و پیلاتس', color: 'bg-teal-500' },
  { icon: Shield, name: 'Martial Arts & Self-Defense Training', namePersian: 'هنرهای رزمی و دفاع شخصی', color: 'bg-gray-600' },
  { icon: Trophy, name: 'Sports-Specific Training', namePersian: 'تمرینات ورزشی تخصصی', color: 'bg-yellow-600' },
  { icon: Heart, name: 'Senior Fitness Coaching', namePersian: 'تناسب اندام سالمندان', color: 'bg-cyan-500' },
  { icon: Baby, name: 'Pre & Postnatal Fitness', namePersian: 'تناسب اندام بارداری', color: 'bg-rose-500' },
  { icon: Utensils, name: 'Nutrition & Meal Planning', namePersian: 'تغذیه و برنامه غذایی', color: 'bg-lime-500' },
  { icon: Target, name: 'Body Transformation Programs', namePersian: 'برنامه تغییر فرم بدن', color: 'bg-emerald-600' },
  { icon: Building, name: 'Corporate & Workplace Fitness', namePersian: 'تناسب اندام شرکتی', color: 'bg-slate-600' }
];

export function FitnessCoachPage({ onNavigate }: FitnessCoachPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [fitnessAds, setFitnessAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadFitnessAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('fitness-coach');
        setFitnessAds(ads);
      } catch (error) {
        console.error('Failed to load fitness coach ads:', error);
        toast.error('Failed to load fitness coach ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedFitnessServices = () => {
    return [...fitnessAds].sort((a, b) => {
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Fitness Coach' : 'مربی تناسب اندام'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search fitness coaches...' : 'جستجوی مربی تناسب اندام...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Fitness Categories' : 'مرور دسته‌بندی مربی‌گری'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Fitness Coaches' : 'مربی‌های تناسب اندام ویژه'}
          </h2>
        ...
        {/* Featured Fitness Services Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : getSortedFitnessServices().length === 0 ? (
            <div className="text-center py-12">
              <Dumbbell className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No fitness coaching services found' 
                  : 'هیچ سرویس مربیگری ورزشی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a fitness coaching service!' 
                  : 'اولین نفری باشید که سرویس مربیگری ورزشی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedFitnessServices().map((service) => (
                <div 
                  key={service.id} 
