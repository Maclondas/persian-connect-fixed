...
import { ArrowLeft, Search, Palette, Target, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
...
const subcategories = [
  { icon: Palette, name: 'Tattoo Artists', namePersian: 'هنرمندان تتو', color: 'bg-purple-500' },
  { icon: Target, name: 'Piercing Services', namePersian: 'خدمات پیرسینگ', color: 'bg-indigo-500' }
];

export function TattooServicesPage({ onNavigate }: TattooServicesPageProps) {
  ...
  const loadAds = async () => {
    try {
      setLoading(true);
      console.log('🔄 TattooServices: Loading ads...');
      
      // Get ads for the "Tattoo Services" category
      const categoryAds = await realDataService.getAdsByCategory('Tattoo Services');
      console.log('📋 TattooServices: Loaded ads:', categoryAds?.length || 0);
      
      setAds(categoryAds || []);
    } catch (error) {
      console.error('❌ TattooServices: Error loading ads:', error);
      toast.error(
        currentLanguage === 'en' 
          ? 'Failed to load tattoo & piercing services' 
          : 'خطا در بارگذاری خدمات تتو و پیرسینگ'
      );
      setAds([]);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Tattoo Services' : 'خدمات تتو'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search tattoo & piercing services...' : 'جستجوی خدمات تتو و پیرسینگ...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Tattoo & Piercing Services' : 'مرور خدمات تتو و پیرسینگ'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Tattoo & Piercing Services' : 'خدمات تتو و پیرسینگ ویژه'}
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {currentLanguage === 'en' 
                  ? 'No tattoo & piercing services available at the moment.' 
                  : 'در حال حاضر خدمات تتو و پیرسینگ موجود نیست.'
                }
              </p>
            </div>
          ) : (
