...
import { ArrowLeft, Search, Languages, FileText, Scale, Stethoscope, Settings, Globe, Mic, Building, GraduationCap, Award, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner';
import { realDataService } from './services/real-data-service';

interface InterpreterTranslationServicesPageProps {
...
const subcategories = [
  { icon: FileText, name: 'Document Translation', namePersian: 'ترجمه اسناد', color: 'bg-blue-500' },
  { icon: Scale, name: 'Legal Translation', namePersian: 'ترجمه حقوقی', color: 'bg-purple-500' },
  { icon: Stethoscope, name: 'Medical Translation', namePersian: 'ترجمه پزشکی', color: 'bg-red-500' },
  { icon: Settings, name: 'Technical Translation', namePersian: 'ترجمه فنی', color: 'bg-gray-500' },
  { icon: Globe, name: 'Website Translation', namePersian: 'ترجمه وب‌سایت', color: 'bg-green-500' },
  { icon: Mic, name: 'Live Interpretation', namePersian: 'ترجمه زنده', color: 'bg-orange-500' },
  { icon: Building, name: 'Conference Interpretation', namePersian: 'ترجمه کنفرانس', color: 'bg-indigo-500' },
  { icon: Building, name: 'Business Translation', namePersian: 'ترجمه تجاری', color: 'bg-teal-500' },
  { icon: GraduationCap, name: 'Academic Translation', namePersian: 'ترجمه دانشگاهی', color: 'bg-yellow-500' },
  { icon: Award, name: 'Certified Translation', namePersian: 'ترجمه رسمی', color: 'bg-pink-500' }
];

export function InterpreterTranslationServicesPage({ onNavigate }: InterpreterTranslationServicesPageProps) {
  ...
  const loadAds = async () => {
    try {
      setLoading(true);
      console.log('🔄 InterpreterTranslation: Loading ads...');
      
      // Get ads for the "Interpreter & Translation Services" category
      const categoryAds = await realDataService.getAdsByCategory('Interpreter & Translation Services');
      console.log('📋 InterpreterTranslation: Loaded ads:', categoryAds?.length || 0);
      
      setAds(categoryAds || []);
    } catch (error) {
      console.error('❌ InterpreterTranslation: Error loading ads:', error);
      toast.error(
        currentLanguage === 'en' 
          ? 'Failed to load translation & interpretation services' 
          : 'خطا در بارگذاری خدمات ترجمه و تفسیر'
      );
      setAds([]);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Interpreter & Translation Services' : 'خدمات ترجمه و تفسیر'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search translation & interpretation services...' : 'جستجوی خدمات ترجمه و تفسیر...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Translation & Interpretation Services' : 'مرور خدمات ترجمه و تفسیر'}
        </h2>
        <div className="grid grid-cols-2 gap-3">
          {subcategories.map((category, index) => {
            ...
            return (
              <button
                key={index}
                className="flex flex-col items-center p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 active:scale-95"
              >
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Translation & Interpretation Services' : 'خدمات ترجمه و تفسیر ویژه'}
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {currentLanguage === 'en' 
                  ? 'No translation & interpretation services available at the moment.' 
                  : 'در حال حاضر خدمات ترجمه و تفسیر موجود نیست.'
                }
              </p>
            </div>
          ) : (
