...
import { ArrowLeft, Search, Car, Truck, Bike, Wrench, Ship, Settings, Zap, Home, Heart, MessageCircle, User, PlusSquare, Check, Calendar, ShoppingCart } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { realDataService as dataService, Ad } from './services/real-data-service';
import { toast } from 'sonner@2.0.3';

interface NavigateFunction {
  (page: 'home' | 'login' | 'ad-detail' | 'post-ad' | 'messages' | 'chat' | 'admin' | 'my-ads' | 'vehicles', params?: { adId?: string; chatId?: string }): void;
}
...
const subcategories = [
  { icon: Car, name: 'Cars for Sale', namePersian: 'خودرو برای فروش', color: 'bg-blue-500' },
  { icon: Calendar, name: 'Car Rentals', namePersian: 'اجاره خودرو', color: 'bg-green-500' },
  { icon: Bike, name: 'Motorcycles & Bicycles', namePersian: 'موتورسیکلت و دوچرخه', color: 'bg-orange-500' },
  { icon: Wrench, name: 'Auto Parts & Accessories', namePersian: 'قطعات و لوازم جانبی', color: 'bg-red-500' },
  { icon: Settings, name: 'Vehicle Services', namePersian: 'خدمات خودرو', color: 'bg-purple-500' },
  { icon: ShoppingCart, name: 'Others', namePersian: 'سایر', color: 'bg-gray-500' }
];
...
export function VehiclesPage({ onNavigate }: VehiclesPageProps) {
  ...
  const loadVehicleAds = async () => {
    try {
      setIsLoading(true);
      
      // Get ads filtered for vehicles category
      const response = await dataService.getAds({
        category: 'vehicles'
      });
      const vehicleAds = response.ads.filter(ad => 
        ad.status === 'approved' &&
        new Date(ad.expiresAt || ad.created_at) > new Date()
      );
      
      setVehicleAds(vehicleAds);
    } catch (error) {
      console.error('Failed to load vehicle ads:', error);
  ...
  const getSortedVehicles = () => {
    const vehicles = [...vehicleAds];
    
    switch (selectedFilter) {
      case 'newest':
        return vehicles.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      case 'oldest':
        return vehicles.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
      case 'lowest-price':
        return vehicles.sort((a, b) => a.price - b.price);
      case 'highest-price':
        return vehicles.sort((a, b) => b.price - a.price);
      case 'urgent':
        return vehicles.sort((a, b) => (b.urgent ? 1 : 0) - (a.urgent ? 1 : 0));
      case 'negotiable':
        return vehicles.sort((a, b) => (b.priceType === 'negotiable' ? 1 : 0) - (a.priceType === 'negotiable' ? 1 : 0));
      default:
        return vehicles;
    }
  };
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Vehicles' : 'خودرو'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search vehicles...' : 'جستجوی خودرو...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Vehicle Categories' : 'مرور دسته‌بندی خودرو'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Vehicle Listings' : 'فهرست خودروها'}
          </h2>
          ...
          {isLoading ? (
            <div className="text-center py-8">
              ...
              <p className="text-gray-500">
                {currentLanguage === 'en' ? 'Loading vehicle listings...' : 'در حال بارگذاری فهرست خودروها...'}
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              ...
              {vehicleAds.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  {currentLanguage === 'en' ? 'No vehicle listings found' : 'فهرست خودرویی یافت نشد'}
                </div>
              )}
            </div>
          )}
