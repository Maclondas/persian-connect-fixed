...
const subcategories = [
  { icon: Stethoscope, name: 'Dentist', namePersian: 'دندانپزشک', color: 'bg-blue-500' },
  { icon: Scissors, name: 'Hair Transplant', namePersian: 'کاشت مو', color: 'bg-green-500' },
  { icon: Heart, name: 'Face & Skin Care', namePersian: 'مراقبت از پوست', color: 'bg-pink-500' },
  { icon: Sparkles, name: 'Nail & Beauty Care', namePersian: 'مراقبت از ناخن', color: 'bg-purple-500' },
  { icon: HandHeart, name: 'Massage Therapy', namePersian: 'ماساژ درمانی', color: 'bg-teal-500' },
  { icon: Syringe, name: 'Filler & Botox', namePersian: 'تزریقات زیبایی', color: 'bg-orange-500' },
  { icon: Microscope, name: 'Dermatology (Skin Specialists)', namePersian: 'متخصص پوست', color: 'bg-red-500' },
  { icon: Zap, name: 'Laser Treatments', namePersian: 'درمان‌های لیزر', color: 'bg-yellow-500' },
  { icon: Activity, name: 'Cosmetic Surgery', namePersian: 'جراحی زیبایی', color: 'bg-indigo-500' },
  { icon: Dumbbell, name: 'Weight Loss & Body Contouring', namePersian: 'کاهش وزن', color: 'bg-lime-500' },
  { icon: Clock, name: 'Anti-Aging Treatments', namePersian: 'ضد پیری', color: 'bg-cyan-500' },
  { icon: Droplets, name: 'IV Drip Therapy', namePersian: 'سرم‌درمانی', color: 'bg-blue-600' },
  { icon: Pill, name: 'Hormone Therapy & Wellness Clinics', namePersian: 'درمان هورمونی', color: 'bg-green-600' },
  { icon: Eye, name: 'Eye & Vision Clinics', namePersian: 'کلینیک چشم', color: 'bg-purple-600' },
  { icon: Ear, name: 'ENT Cosmetic Treatments', namePersian: 'زیبایی گوش و حلق', color: 'bg-pink-600' },
  { icon: Brush, name: 'Permanent Makeup', namePersian: 'آرایش دائم', color: 'bg-red-600' },
  { icon: Paintbrush, name: 'Eyelash Extensions & Lifts', namePersian: 'اکستنشن مژه', color: 'bg-orange-600' },
  { icon: Eraser, name: 'Tattoo Removal', namePersian: 'حذف تتو', color: 'bg-gray-600' },
  { icon: ArrowUpDown, name: 'Thread Lifting & Non-Surgical Facelift', namePersian: 'لیفت نخی', color: 'bg-teal-600' },
  { icon: Sun, name: 'Skin Tightening & Resurfacing', namePersian: 'سفت‌کردن پوست', color: 'bg-yellow-600' },
  { icon: Star, name: 'Acne & Scar Treatments', namePersian: 'درمان آکنه', color: 'bg-indigo-600' },
  { icon: Plus, name: 'Dental Aesthetics', namePersian: 'زیبایی دندان', color: 'bg-cyan-600' }
];

export function DoctorsBeautyClinicsPage({ onNavigate }: DoctorsBeautyClinicsPageProps) {
  ...
  const loadAds = async () => {
    try {
      setLoading(true);
      console.log('🔄 DoctorsBeautyClinics: Loading ads...');
      
      // Get ads for the "Doctors & Beauty Clinics" category
      const categoryAds = await realDataService.getAdsByCategory('Doctors & Beauty Clinics');
      console.log('📋 DoctorsBeautyClinics: Loaded ads:', categoryAds?.length || 0);
      
      setAds(categoryAds || []);
    } catch (error) {
      console.error('❌ DoctorsBeautyClinics: Error loading ads:', error);
      toast.error(
        currentLanguage === 'en' 
          ? 'Failed to load medical & beauty services' 
          : 'خطا در بارگذاری خدمات پزشکی و زیبایی'
      );
      setAds([]);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Doctors & Beauty Clinics' : 'پزشکان و کلینیک‌های زیبایی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search medical & beauty services...' : 'جستجوی خدمات پزشکی و زیبایی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Medical & Beauty Categories' : 'مرور دسته‌بندی خدمات پزشکی و زیبایی'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Medical & Beauty Services' : 'خدمات پزشکی و زیبایی ویژه'}
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {currentLanguage === 'en' 
                  ? 'No medical & beauty services available at the moment.' 
                  : 'در حال حاضر خدمات پزشکی و زیبایی‌ای موجود نیست.'
                }
              </p>
            </div>
          ) : (
