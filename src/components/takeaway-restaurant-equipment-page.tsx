...
const subcategories = [
  { icon: ChefHat, name: 'Cooking Equipment', namePersian: 'تجهیزات پخت', color: 'bg-orange-500' },
  { icon: Snowflake, name: 'Refrigeration & Freezers', namePersian: 'یخچال و فریزر', color: 'bg-blue-500' },
  { icon: Utensils, name: 'Food Preparation Equipment', namePersian: 'تجهیزات آماده‌سازی غذا', color: 'bg-green-500' },
  { icon: Coffee, name: 'Beverage Equipment', namePersian: 'تجهیزات نوشیدنی', color: 'bg-brown-500' },
  { icon: Thermometer, name: 'Heating & Holding Equipment', namePersian: 'تجهیزات گرمایشی', color: 'bg-red-500' },
  { icon: Package, name: 'Packaging & Disposables', namePersian: 'بسته‌بندی و یکبار مصرف', color: 'bg-purple-500' },
  { icon: Presentation, name: 'Serving & Display Equipment', namePersian: 'تجهیزات سرو و نمایش', color: 'bg-pink-500' },
  { icon: Sparkles, name: 'Cleaning & Hygiene Equipment', namePersian: 'تجهیزات نظافت', color: 'bg-teal-500' },
  { icon: Sofa, name: 'Furniture & Fixtures', namePersian: 'مبلمان و تجهیزات', color: 'bg-indigo-500' },
  { icon: CreditCard, name: 'POS & Ordering Systems', namePersian: 'سیستم فروش و سفارش', color: 'bg-yellow-500' },
  { icon: Wind, name: 'Ventilation & Extraction Systems', namePersian: 'سیستم تهویه', color: 'bg-cyan-500' },
  { icon: Truck, name: 'Delivery & Storage Equipment', namePersian: 'تجهیزات تحویل و انبار', color: 'bg-lime-500' }
];

export function TakeawayRestaurantEquipmentPage({ onNavigate }: TakeawayRestaurantEquipmentPageProps) {
  ...
  const loadAds = async () => {
    try {
      setLoading(true);
      console.log('🔄 TakeawayRestaurantEquipment: Loading ads...');
      
      // Get ads for the "Takeaway & Restaurant Equipment" category
      const categoryAds = await realDataService.getAdsByCategory('Takeaway & Restaurant Equipment');
      console.log('📋 TakeawayRestaurantEquipment: Loaded ads:', categoryAds?.length || 0);
      
      setAds(categoryAds || []);
    } catch (error) {
      console.error('❌ TakeawayRestaurantEquipment: Error loading ads:', error);
      toast.error(
        currentLanguage === 'en' 
          ? 'Failed to load restaurant equipment ads' 
          : 'خطا در بارگذاری آگهی‌های تجهیزات رستوران'
      );
      setAds([]);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Takeaway & Restaurant Equipment' : 'تجهیزات رستوران و بیرون‌بر'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search restaurant equipment...' : 'جستجوی تجهیزات رستوران...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Equipment Categories' : 'مرور دسته‌بندی تجهیزات'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Restaurant Equipment' : 'تجهیزات رستوران ویژه'}
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {currentLanguage === 'en' 
                  ? 'No restaurant equipment available at the moment.' 
                  : 'در حال حاضر تجهیزات رستورانی موجود نیست.'
                }
              </p>
            </div>
          ) : (
