...
import { ArrowLeft, Search, ShoppingCart, Store, Candy, Utensils, Building, Cake, Beef, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner@2.0.3';
import { realDataService, Ad } from './services/real-data-service';

interface SupermarketWholesalersPageProps {
...
const subcategories = [
  { icon: Store, name: 'Persian Supermarket', namePersian: 'سوپرمارکت ایرانی', color: 'bg-green-500' },
  { icon: Candy, name: 'Persian Sweet Shop', namePersian: 'قنادی ایرانی', color: 'bg-pink-500' },
  { icon: Utensils, name: 'Restaurant & Takeaway Wholesalers', namePersian: 'عمده فروشی رستوران', color: 'bg-orange-500' },
  { icon: Building, name: 'Hypermarkets', namePersian: 'هایپرمارکت', color: 'bg-blue-500' },
  { icon: Cake, name: 'Persian Bakery', namePersian: 'نانوایی ایرانی', color: 'bg-yellow-500' },
  { icon: Beef, name: 'Persian Butchers', namePersian: 'قصابی ایرانی', color: 'bg-red-500' }
];

export function SupermarketWholesalersPage({ onNavigate }: SupermarketWholesalersPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [supermarketAds, setSupermarketAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadSupermarketAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('supermarket-wholesalers');
        setSupermarketAds(ads);
      } catch (error) {
        console.error('Failed to load supermarket ads:', error);
        toast.error('Failed to load supermarket ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedSupermarketServices = () => {
    const services = [...supermarketAds];
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Supermarkets & Wholesalers' : 'سوپرمارکت و عمده فروشی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search supermarkets & wholesalers...' : 'جستجوی سوپرمارکت و عمده فروشی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Supermarket & Wholesale Categories' : 'مرور دسته‌بندی سوپرمارکت و عمده فروشی'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Supermarket & Wholesale Services' : 'خدمات سوپرمارکت و عمده فروشی ویژه'}
          </h2>
        ...
        {/* Supermarket Services Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : supermarketAds.length === 0 ? (
            <div className="text-center py-12">
              <ShoppingCart className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No supermarket services found' 
                  : 'هیچ خدمات سوپرمارکتی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a supermarket service!' 
                  : 'اولین نفری باشید که خدمات سوپرمارکت ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedSupermarketServices().map((ad) => (
                <div 
                  key={ad.id} 
