...
import { ArrowLeft, Search, Coins, ArrowLeftRight, Gem, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
...
const subcategories = [
  { icon: ArrowLeftRight, name: 'Currency Exchanger', namePersian: 'صرافی ارز', color: 'bg-blue-500' },
  { icon: Gem, name: 'Jewellery Shop', namePersian: 'طلا و جواهرفروشی', color: 'bg-purple-500' }
];

export function CurrencyExchangePage({ onNavigate }: CurrencyExchangePageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [currencyAds, setCurrencyAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadCurrencyAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('currency-exchange');
        setCurrencyAds(ads);
      } catch (error) {
        console.error('Failed to load currency exchange ads:', error);
        toast.error('Failed to load currency exchange ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedCurrencyServices = () => {
    const services = [...currencyAds];
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Currency Exchange' : 'صرافی و طلا فروشی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search currency & jewellery services...' : 'جستجوی خدمات صرافی و طلافروشی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Currency & Jewellery Categories' : 'مرور دسته‌بندی صرافی و طلافروشی'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Currency & Jewellery Services' : 'خدمات صرافی و طلافروشی ویژه'}
          </h2>
        ...
        {/* Featured Currency Services Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : getSortedCurrencyServices().length === 0 ? (
            <div className="text-center py-12">
              <Coins className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No currency exchange services found' 
                  : 'هیچ سرویس صرافی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a currency exchange service!' 
                  : 'اولین نفری باشید که سرویس صرافی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedCurrencyServices().map((service) => (
                <div 
                  key={service.id} 
