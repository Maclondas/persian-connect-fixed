...
import { ArrowLeft, Search, Car, Settings, Zap, BookOpen, CheckCircle, RefreshCw, Truck, ParkingCircle, Users, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner';
import { realDataService, Ad } from './services/real-data-service';

interface DrivingLessonsPageProps {
...
const subcategories = [
  { icon: Car, name: 'Manual Transmission Lessons', namePersian: 'آموزش گیربکس دستی', color: 'bg-blue-500' },
  { icon: Settings, name: 'Automatic Transmission Lessons', namePersian: 'آموزش گیربکس اتوماتیک', color: 'bg-green-500' },
  { icon: Zap, name: 'Intensive / Crash Courses', namePersian: 'دوره‌های فشرده', color: 'bg-orange-500' },
  { icon: BookOpen, name: 'Theory Test Preparation', namePersian: 'آمادگی برای آزمون تئوری', color: 'bg-purple-500' },
  { icon: CheckCircle, name: 'Practical Test Preparation', namePersian: 'آمادگی برای آزمون عملی', color: 'bg-red-500' },
  { icon: RefreshCw, name: 'Refresher Lessons', namePersian: 'کلاس‌های تجدید مهارت', color: 'bg-indigo-500' },
  { icon: Truck, name: 'Motorway Lessons', namePersian: 'آموزش رانندگی در بزرگراه', color: 'bg-gray-600' },
  { icon: ParkingCircle, name: 'Parking Lessons', namePersian: 'آموزش پارک کردن', color: 'bg-cyan-500' },
  { icon: Users, name: 'Nervous Driver Support', namePersian: 'حمایت از رانندگان مضطرب', color: 'bg-teal-500' }
];

export function DrivingLessonsPage({ onNavigate }: DrivingLessonsPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [drivingAds, setDrivingAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadDrivingAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('driving-lessons');
        setDrivingAds(ads);
      } catch (error) {
        console.error('Failed to load driving lesson ads:', error);
        toast.error('Failed to load driving lessons');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedDrivingLessons = () => {
    const lessons = [...drivingAds];
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Driving Lessons' : 'آموزش رانندگی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search driving lessons...' : 'جستجوی کلاس‌های رانندگی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Driving Lesson Categories' : 'مرور دسته‌بندی کلاس‌های رانندگی'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Driving Lessons' : 'کلاس‌های رانندگی ویژه'}
          </h2>
        ...
        {/* Featured Driving Lessons Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : getSortedDrivingLessons().length === 0 ? (
            <div className="text-center py-12">
              <Car className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No driving lessons found' 
                  : 'هیچ آموزش رانندگی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a driving lesson!' 
                  : 'اولین نفری باشید که آموزش رانندگی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedDrivingLessons().map((lesson) => (
                <div 
                  key={lesson.id} 
                  ...
                  onClick={() => onNavigate('ad-detail', { adId: lesson.id })}
                >
                  <div className="flex">
                    <div className="relative w-24 h-24 flex-shrink-0">
                      <ImageWithFallback
                        src={lesson.imageUrl || '/placeholder-ad.jpg'}
                        alt={currentLanguage === 'en' ? lesson.title : lesson.titlePersian}
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          <h3 className="font-semibold text-sm mb-1 line-clamp-1">
                            {currentLanguage === 'en' ? lesson.title : lesson.titlePersian}
                          </h3>
                          <p className="text-xs text-gray-600 mb-1">{lesson.contact?.name || 'N/A'}</p>
                          <p className="text-xs text-gray-500 mb-1">
                            {lesson.location?.city ? `${lesson.location.city}, ${lesson.location.country}` : 'Location not specified'}
                          </p>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-[#0ac2af]">
                            {lesson.price ? `£${lesson.price}` : 'Contact for price'}
                          </span>
                          {lesson.negotiable && (
