...
import { ArrowLeft, Search, Scale, Users, Home as HomeIcon, Building, Briefcase, Shield, FileText, HelpCircle, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner';
import { realDataService, Ad } from './services/real-data-service';

interface SolicitorLegalServicesPageProps {
...
const subcategories = [
  { icon: Scale, name: 'Immigration Law', namePersian: 'قانون مهاجرت', color: 'bg-blue-500' },
  { icon: Users, name: 'Family Law', namePersian: 'قانون خانواده', color: 'bg-green-500' },
  { icon: HomeIcon, name: 'Property Law / Conveyancing', namePersian: 'قانون املاک', color: 'bg-purple-500' },
  { icon: Building, name: 'Business & Commercial Law', namePersian: 'قانون تجاری', color: 'bg-red-500' },
  { icon: Shield, name: 'Criminal Law', namePersian: 'قانون جنایی', color: 'bg-orange-500' },
  { icon: Heart, name: 'Personal Injury Claims', namePersian: 'ادعای آسیب شخصی', color: 'bg-teal-500' },
  { icon: FileText, name: 'Wills, Trusts & Probate', namePersian: 'وصیت‌نامه و ارث', color: 'bg-indigo-500' },
  { icon: HelpCircle, name: 'General Legal Advice', namePersian: 'مشاوره حقوقی عمومی', color: 'bg-pink-500' }
];

export function SolicitorLegalServicesPage({ onNavigate }: SolicitorLegalServicesPageProps) {
  const { currentLanguage } = useLanguage();
  const [selectedFilter, setSelectedFilter] = useState<FilterOption>('newest');
  const [showFilterOptions, setShowFilterOptions] = useState(false);
  const [legalAds, setLegalAds] = useState<Ad[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const loadLegalAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('legal');
        setLegalAds(ads);
      } catch (error) {
        console.error('Failed to load legal services ads:', error);
        toast.error('Failed to load legal services ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedLegalServices = () => {
    return [...legalAds].sort((a, b) => {
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Solicitor & Legal Services' : 'خدمات حقوقی و وکلا'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              type="search"
              placeholder={currentLanguage === 'en' ? 'Search legal services...' : 'جستجو خدمات حقوقی...'}
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Legal Services' : 'خدمات حقوقی ویژه'}
          </h2>
        ...
        {/* Featured Legal Services Grid */}
        <div className="px-4 py-4">
          {isLoading ? (
            <div className="flex justify-center items-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
            </div>
          ) : getSortedLegalServices().length === 0 ? (
            <div className="text-center py-12">
              <Scale className="h-12 w-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No legal services found' 
                  : 'هیچ سرویس حقوقی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post a legal service!' 
                  : 'اولین نفری باشید که سرویس حقوقی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedLegalServices().map((service) => (
                <div 
                  key={service.id} 
