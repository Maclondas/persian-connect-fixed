...
import { ArrowLeft, Search, Scissors, Sparkles, Heart, MessageCircle, User, PlusSquare, Check, Home } from 'lucide-react';
import { useLanguage } from './hooks/useLanguage';
import { toast } from 'sonner';
import { realDataService } from './services/real-data-service';

interface BarberHairdresserPageProps {
...
const subcategories = [
  { icon: Scissors, name: 'Men\'s Barber Shop', namePersian: 'آرایشگاه مردانه', color: 'bg-blue-500' },
  { icon: Sparkles, name: 'Women\'s Hairdresser', namePersian: 'آرایشگاه زنانه', color: 'bg-pink-500' }
];

export function BarberHairdresserPage({ onNavigate }: BarberHairdresserPageProps) {
  ...
  const loadAds = async () => {
    try {
      setLoading(true);
      console.log('🔄 BarberHairdresser: Loading ads...');
      
      // Get ads for the "Barber Shop & Hairdresser" category
      const categoryAds = await realDataService.getAdsByCategory('Barber Shop & Hairdresser');
      console.log('📋 BarberHairdresser: Loaded ads:', categoryAds?.length || 0);
      
      setAds(categoryAds || []);
    } catch (error) {
      console.error('❌ BarberHairdresser: Error loading ads:', error);
      toast.error(
        currentLanguage === 'en' 
          ? 'Failed to load hair & grooming services' 
          : 'خطا در بارگذاری خدمات آرایشی و مو'
      );
      setAds([]);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Barber Shop & Hairdresser' : 'آرایشگاه مردانه و زنانه'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search hair & grooming services...' : 'جستجوی خدمات آرایشی و مو...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Hair & Grooming Services' : 'مرور خدمات آرایشی و مو'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Hair & Grooming Services' : 'خدمات آرایشی و مو ویژه'}
          </h2>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">
                {currentLanguage === 'en' 
                  ? 'No hair & grooming services available at the moment.' 
                  : 'در حال حاضر خدمات آرایشی و مو موجود نیست.'
                }
              </p>
            </div>
          ) : (
