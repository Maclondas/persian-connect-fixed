...
import { ArrowLeft, Search, Heart, MessageCircle, User, PlusSquare, Check, Home, Dog, Package, UtensilsCrossed, Scissors, Fish, Bird, Rabbit, Stethoscope } from 'lucide-react';
...
const subcategories = [
  { icon: Dog, name: 'Dogs', namePersian: 'سگ‌ها', color: 'bg-blue-500' },
  { icon: Heart, name: 'Cats', namePersian: 'گربه‌ها', color: 'bg-green-500' },
  { icon: Bird, name: 'Birds', namePersian: 'پرندگان', color: 'bg-orange-500' },
  { icon: Fish, name: 'Fish & Aquariums', namePersian: 'ماهی و آکواریوم', color: 'bg-cyan-500' },
  { icon: Rabbit, name: 'Small Animals', namePersian: 'حیوانات کوچک', color: 'bg-purple-500' },
  { icon: Package, name: 'Pet Food & Supplies', namePersian: 'غذا و لوازم حیوانات', color: 'bg-red-500' },
  { icon: Stethoscope, name: 'Pet Services', namePersian: 'خدمات حیوانات', color: 'bg-gray-500' }
];
...
export function PetsPage({ onNavigate }: PetsPageProps) {
  ...
  useEffect(() => {
    const loadPetAds = async () => {
      try {
        setIsLoading(true);
        const ads = await realDataService.getAdsByCategory('pets');
        setPetAds(ads);
      } catch (error) {
        console.error('Failed to load pet ads:', error);
        toast.error('Failed to load pet ads');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const getSortedPets = () => {
    ...
    switch (selectedFilter) {
      ...
      case 'oldest':
        return pets.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
      case 'lowest-price':
        return pets.sort((a, b) => (a.price || 0) - (b.price || 0));
      case 'highest-price':
        return pets.sort((a, b) => (b.price || 0) - (a.price || 0));
      case 'urgent':
        return pets.sort((a, b) => (b.isUrgent ? 1 : 0) - (a.isUrgent ? 1 : 0));
      case 'negotiable':
        return pets.sort((a, b) => (b.negotiable ? 1 : 0) - (a.negotiable ? 1 : 0));
      default:
        return pets;
    }
  };
    ...
    <div className="min-h-screen bg-white text-black" dir={currentLanguage === 'fa' ? 'rtl' : 'ltr'}>
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <h1 className="text-lg font-semibold text-center flex-1 mx-4">
            {currentLanguage === 'en' ? 'Pets & Animals' : 'حیوانات خانگی'}
          </h1>
        ...
        <div className="px-4 pb-3">
          <div className="relative">
            ...
            <Input
              placeholder={currentLanguage === 'en' ? 'Search pets & animals...' : 'جستجوی حیوانات خانگی...'}
      ...
      <div className="px-4 py-6 bg-gray-50">
        <h2 className="text-lg font-semibold mb-4">
          {currentLanguage === 'en' ? 'Browse Pet Categories' : 'مرور دسته‌بندی حیوانات'}
        </h2>
      ...
      <div className="bg-white">
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-100">
          <h2 className="text-lg font-semibold">
            {currentLanguage === 'en' ? 'Featured Pets' : 'حیوانات ویژه'}
          </h2>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 mb-2">
                {currentLanguage === 'en' 
                  ? 'No pets found' 
                  : 'هیچ حیوان خانگی یافت نشد'
                }
              </p>
              <p className="text-sm text-gray-400">
                {currentLanguage === 'en' 
                  ? 'Be the first to post pets & animals!' 
                  : 'اولین نفری باشید که حیوان خانگی ارسال می‌کنید!'
                }
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {getSortedPets().map((ad) => (
                <div 
                  key={ad.id} 
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-[#0ac2af]">
                            {ad.price ? (ad.price === 0 ? 'Free' : `£${ad.price}`) : (currentLanguage === 'en' ? 'Contact for price' : 'تماس برای قیمت')}
                          </span>
                          {ad.negotiable && (
